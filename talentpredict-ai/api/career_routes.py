"""Career AI routes: interview simulation and global opportunities discovery."""

from __future__ import annotations

import asyncio
import io
import json
import logging
import math
import re
from copy import deepcopy
from datetime import date, datetime, timedelta, timezone
from typing import Any
from urllib.parse import quote_plus, urljoin

import httpx
import PyPDF2
from bs4 import BeautifulSoup
from fastapi import APIRouter
from pydantic import AliasChoices, BaseModel, ConfigDict, Field, field_validator

from services.job_description_match import match_job_description
from services.ollama_client import call_ollama_json

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/career", tags=["career"])

_DEFAULT_PLATFORMS = [
    "linkedin",
    "upwork",
    "indeed",
    "glassdoor",
    "wellfound",
    "freelancer",
    "weworkremotely",
    "remoteok",
    "remotive",
    "arbeitnow",
]

_SUPPORTED_PLATFORMS = set(_DEFAULT_PLATFORMS)

_HTTP_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/124.0.0.0 Safari/537.36 TalentPredictBot/1.0"
    )
}

_URL_HINTS_BY_PLATFORM: dict[str, list[str]] = {
    "linkedin": ["/jobs/view", "linkedin.com/jobs/"],
    "upwork": ["/jobs/~", "/freelance-jobs/", "/nx/search/jobs/"],
    "indeed": ["/viewjob", "jk="],
    "glassdoor": ["/job-listing", "/job/"],
    "wellfound": ["/jobs/"],
    "freelancer": ["/projects/", "/jobs/"],
    "weworkremotely": ["/remote-jobs/"],
}


class InterviewGenerateBody(BaseModel):
    candidate_id: str
    target_role: str = "Software Engineer"
    level: str = "INTERMEDIATE"
    interview_type: str = "mixed"
    language: str = "english"
    question_count: int = Field(default=8, ge=4, le=15)
    focus_skills: list[str] = Field(default_factory=list)
    strong_skills: list[str] = Field(default_factory=list)
    weak_skills: list[str] = Field(default_factory=list)
    job_description: str | None = None


class InterviewEvaluateBody(BaseModel):
    candidate_id: str
    role: str = "Software Engineer"
    level: str = "INTERMEDIATE"
    category: str = "mixed"
    question: str
    answer: str
    ideal_answer_points: list[str] = Field(default_factory=list)
    job_description: str | None = None


class OpportunityDiscoverBody(BaseModel):
    candidate_id: str
    position: str = "Software Engineer"
    level: str = "INTERMEDIATE"
    location: str = "Remote"
    remote_only: bool = True
    platforms: list[str] = Field(default_factory=list)
    max_results: int = Field(default=24, ge=6, le=80)
    job_description: str | None = None
    candidate_skills: list[dict[str, Any]] = Field(default_factory=list)
    candidate_context: dict[str, Any] = Field(default_factory=dict)


class ProfileImproveBody(BaseModel):
    candidate_id: str
    target_role: str = "Software Engineer"
    level: str = "INTERMEDIATE"
    current_headline: str | None = None
    current_summary: str | None = None
    current_skills: list[str] = Field(default_factory=list)
    linkedin_url: str | None = None
    linkedin_about: str | None = None
    cv_url: str | None = None
    cv_text: str | None = None
    language: str = "english"
    job_description: str | None = None
    candidate_skills: list[dict[str, Any]] = Field(default_factory=list)
    candidate_context: dict[str, Any] = Field(default_factory=dict)


async def _safe_get_json(url: str) -> Any:
    async with httpx.AsyncClient(timeout=25.0, follow_redirects=True, headers=_HTTP_HEADERS) as client:
        response = await client.get(url)
        response.raise_for_status()
        return response.json()


async def _safe_get_text(url: str) -> str:
    async with httpx.AsyncClient(timeout=25.0, follow_redirects=True, headers=_HTTP_HEADERS) as client:
        response = await client.get(url)
        response.raise_for_status()
        return response.text


async def _extract_cv_text_from_url(cv_url: str | None, max_chars: int = 6000) -> str:
    url = (cv_url or "").strip()
    if not url or not url.lower().startswith(("http://", "https://")):
        return ""

    try:
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True, headers=_HTTP_HEADERS) as client:
            response = await client.get(url)
            response.raise_for_status()
            file_bytes = response.content

        if not file_bytes:
            return ""

        reader = PyPDF2.PdfReader(io.BytesIO(file_bytes))
        parts: list[str] = []
        total = 0
        for page in reader.pages[:8]:
            page_text = (page.extract_text() or "").strip()
            if not page_text:
                continue

            remaining = max_chars - total
            if remaining <= 0:
                break

            chunk = page_text[:remaining]
            if not chunk:
                continue

            parts.append(chunk)
            total += len(chunk)

        return _clean_text("\n".join(parts), max_chars)
    except Exception as exc:
        logger.warning("CV URL extraction failed for %s: %s", url, exc)
        return ""


def _normalize_level(raw_level: str | None) -> str:
    value = (raw_level or "INTERMEDIATE").strip().upper()
    if value in {"BEGINNER", "JUNIOR"}:
        return "BEGINNER"
    if value in {"INTERMEDIATE", "MID", "MIDDLE"}:
        return "INTERMEDIATE"
    if value in {"ADVANCED", "SENIOR"}:
        return "ADVANCED"
    if value in {"EXPERT", "LEAD", "PRINCIPAL", "STAFF"}:
        return "EXPERT"
    return "INTERMEDIATE"


def _normalize_platforms(platforms: list[str]) -> list[str]:
    if not platforms:
        return list(_DEFAULT_PLATFORMS)

    normalized: list[str] = []
    for platform in platforms:
        p = platform.strip().lower()
        if p in _SUPPORTED_PLATFORMS and p not in normalized:
            normalized.append(p)

    return normalized or list(_DEFAULT_PLATFORMS)


def _split_keywords(text: str) -> list[str]:
    parts = re.split(r"[^a-zA-Z0-9\+#.-]+", text.lower())
    return [p for p in parts if len(p) >= 3]


def _build_query(position: str, level: str, location: str, remote_only: bool) -> str:
    level_norm = _normalize_level(level)
    level_label = {
        "BEGINNER": "junior",
        "INTERMEDIATE": "mid",
        "ADVANCED": "senior",
        "EXPERT": "lead",
    }[level_norm]
    remote_label = "remote" if remote_only else ""
    chunks = [position.strip(), level_label, location.strip(), remote_label]
    return " ".join(part for part in chunks if part)


def _clean_text(value: str, max_len: int = 280) -> str:
    compact = " ".join(value.split())
    if len(compact) <= max_len:
        return compact
    return compact[: max_len - 3].rstrip() + "..."


def _string_list(value: Any, max_items: int = 12) -> list[str]:
    if not isinstance(value, list):
        return []

    items: list[str] = []
    seen: set[str] = set()
    for raw in value:
        text = str(raw).strip()
        if not text:
            continue
        key = text.lower()
        if key in seen:
            continue
        seen.add(key)
        items.append(text)
        if len(items) >= max_items:
            break

    return items


def _actionable_edits(value: Any, max_items: int = 8) -> list[dict[str, str]]:
    if not isinstance(value, list):
        return []

    edits: list[dict[str, str]] = []
    for raw in value:
        if not isinstance(raw, dict):
            continue
        action = _clean_text(str(raw.get("action", "")).strip(), 220)
        before = _clean_text(str(raw.get("before", "")).strip(), 220)
        after = _clean_text(str(raw.get("after", "")).strip(), 220)
        why = _clean_text(str(raw.get("why", "")).strip(), 220)
        if not action and not after:
            continue
        edits.append(
            {
                "action": action,
                "before": before,
                "after": after,
                "why": why,
            }
        )
        if len(edits) >= max_items:
            break

    return edits


def _normalize_skill_names(raw_skills: list[str], candidate_skills: list[dict[str, Any]]) -> list[str]:
    names: list[str] = []
    seen: set[str] = set()

    for value in raw_skills:
        text = str(value).strip()
        if not text:
            continue
        key = text.lower()
        if key in seen:
            continue
        seen.add(key)
        names.append(text)

    for row in candidate_skills:
        if not isinstance(row, dict):
            continue
        text = str(row.get("name", "")).strip()
        if not text:
            continue
        key = text.lower()
        if key in seen:
            continue
        seen.add(key)
        names.append(text)

    return names[:20]


def _summary_seed_from_cv(cv_text: str | None, max_len: int = 520) -> str:
    text = (cv_text or "").strip()
    if not text:
        return ""

    sentences = re.split(r"(?<=[.!?])\s+|\n+", text)
    selected: list[str] = []
    total = 0

    for raw_sentence in sentences:
        sentence = " ".join(raw_sentence.split()).strip(" -")
        if len(sentence) < 30:
            continue
        if total + len(sentence) > max_len:
            break
        selected.append(sentence)
        total += len(sentence) + 1
        if len(selected) >= 3:
            break

    if not selected:
        return _clean_text(text, max_len)

    return _clean_text(" ".join(selected), max_len)


def _build_profile_improvement_fallback(body: ProfileImproveBody, cv_text_override: str | None = None) -> dict[str, Any]:
    target_role = body.target_role.strip() or "Software Engineer"
    skills = _normalize_skill_names(body.current_skills, body.candidate_skills)
    core_skills = skills[:8]

    current_headline = _clean_text((body.current_headline or "").strip() or target_role, 160)
    top_skill_phrase = ", ".join(core_skills[:3]) if core_skills else "problem solving, communication, delivery"
    improved_headline = _clean_text(
        f"{target_role} | {top_skill_phrase} | Delivering measurable product impact",
        170,
    )

    summary_seed = _summary_seed_from_cv(cv_text_override if cv_text_override is not None else body.cv_text, 520)
    summary_source = (body.current_summary or body.linkedin_about or summary_seed).strip()
    years_raw = body.candidate_context.get("total_experience_years") if isinstance(body.candidate_context, dict) else None
    years_text = f"{years_raw}+ years" if isinstance(years_raw, int) and years_raw > 0 else "hands-on"
    improved_summary = _clean_text(
        (
            f"{target_role} with {years_text} of experience building reliable solutions. "
            f"Strengths include {', '.join(core_skills[:4]) if core_skills else 'cross-functional execution'} "
            "and translating business goals into clear technical outcomes with measurable impact."
        ),
        520,
    )

    role_terms = _split_keywords(target_role)
    add_suggestions = [
        "system design",
        "stakeholder communication",
        "impact metrics",
        "testing strategy",
    ]
    if "frontend" in role_terms:
        add_suggestions.extend(["web performance", "accessibility"])
    if "backend" in role_terms:
        add_suggestions.extend(["api scalability", "observability"])
    if "data" in role_terms:
        add_suggestions.extend(["sql optimization", "data modeling"])

    dedup_add = []
    seen_add: set[str] = set(skill.lower() for skill in core_skills)
    for suggestion in add_suggestions:
        key = suggestion.lower()
        if key in seen_add:
            continue
        seen_add.add(key)
        dedup_add.append(suggestion)

    deprioritize = skills[8:12]

    alignment_score = 42
    if core_skills:
        alignment_score += min(28, len(core_skills) * 4)
    if summary_source:
        alignment_score += 9
    if body.current_headline and target_role.lower() in body.current_headline.lower():
        alignment_score += 8
    if body.job_description and body.job_description.strip():
        alignment_score += 8
    alignment_score = max(20, min(96, alignment_score))

    strengths: list[str] = []
    if core_skills:
        strengths.append(f"Role-relevant skills already visible: {', '.join(core_skills[:4])}.")
    if summary_source:
        strengths.append("You already provide narrative material that can be reframed for impact.")

    gaps: list[str] = []
    if not body.current_headline:
        gaps.append("Headline does not yet communicate specialization and business impact.")
    if not summary_source:
        gaps.append("Summary/About section is missing quantified outcomes.")
    if len(core_skills) < 5:
        gaps.append("Skills list is too narrow for confident role targeting.")

    return {
        "target_role": target_role,
        "headline": {
            "current": current_headline,
            "improved": improved_headline,
            "actionable_edits": [
                {
                    "action": "Lead with role specialization",
                    "before": current_headline,
                    "after": improved_headline,
                    "why": "Recruiters scan role fit in the first 2 seconds.",
                },
                {
                    "action": "Add 2-3 core skills in headline",
                    "before": current_headline,
                    "after": improved_headline,
                    "why": "Concrete keywords improve search visibility.",
                },
                {
                    "action": "Include impact wording",
                    "before": current_headline,
                    "after": improved_headline,
                    "why": "Impact language signals senior ownership.",
                },
            ],
        },
        "summary": {
            "current": _clean_text(summary_source, 520),
            "improved": improved_summary,
            "actionable_edits": [
                {
                    "action": "Open with clear positioning",
                    "before": _clean_text(summary_source[:180], 180),
                    "after": _clean_text(improved_summary[:220], 220),
                    "why": "A strong opening clarifies target role immediately.",
                },
                {
                    "action": "Quantify outcomes",
                    "before": "General statements without metrics",
                    "after": "Include outcomes like delivery speed, uptime, or cost impact",
                    "why": "Quantified outcomes increase credibility.",
                },
                {
                    "action": "Connect strengths to business value",
                    "before": "List tasks performed",
                    "after": "Show how your work improved product or team results",
                    "why": "Business context differentiates candidates with similar stacks.",
                },
            ],
        },
        "skills": {
            "current": skills,
            "prioritized_for_role": core_skills,
            "add": dedup_add[:8],
            "deprioritize": deprioritize,
            "actionable_edits": [
                {
                    "action": "Move role-critical skills to top 10",
                    "before": ", ".join(skills[:10]) if skills else "Unstructured skill list",
                    "after": ", ".join(core_skills[:10]) if core_skills else "Role-specific top skills",
                    "why": "Top-positioned skills shape first impressions and ATS matching.",
                },
                {
                    "action": "Add missing complementary skills",
                    "before": "Only core technical stack",
                    "after": ", ".join(dedup_add[:5]) if dedup_add else "Include role-complementary skills",
                    "why": "Breadth improves alignment with real job descriptions.",
                },
            ],
        },
        "role_alignment": {
            "score": alignment_score,
            "strengths": strengths,
            "gaps": gaps,
            "actionable_edits": [
                {
                    "action": "Align headline + summary + top skills to one target role",
                    "before": "Mixed signals across profile sections",
                    "after": f"Consistent positioning around {target_role}",
                    "why": "Consistency improves recruiter confidence and interview conversion.",
                },
                {
                    "action": "Tailor wording to target role terminology",
                    "before": "Generic language",
                    "after": f"Use terms commonly seen in {target_role} job posts",
                    "why": "Terminology alignment improves search and relevance scoring.",
                },
            ],
        },
        "next_steps": [
            "Update headline and About section first, then refresh pinned experience bullets.",
            "Reorder skills so the first 10 directly support the target role.",
            "Run one interview simulation using the updated profile language.",
        ],
    }


def _job_like_url(platform: str, url: str) -> bool:
    hints = _URL_HINTS_BY_PLATFORM.get(platform, [])
    lowered = url.lower()
    return any(hint in lowered for hint in hints)


def _score_opportunity(
    item: dict[str, Any],
    candidate_skills: list[dict[str, Any]],
    query_terms: list[str],
) -> tuple[int, list[str]]:
    text = " ".join([
        str(item.get("title", "")),
        str(item.get("company", "")),
        str(item.get("snippet", "")),
    ]).lower()

    skill_hits: list[str] = []
    for skill in candidate_skills:
        name = str(skill.get("name", "")).strip().lower()
        if len(name) < 2:
            continue
        if name in text and name not in skill_hits:
            skill_hits.append(name)

    query_hits = len([term for term in query_terms if term in text])
    base = 35
    score = base + min(45, len(skill_hits) * 8) + min(20, query_hits * 3)
    return max(0, min(99, score)), skill_hits[:6]


def _to_opportunity(
    *,
    platform: str,
    title: str,
    company: str,
    location: str,
    url: str,
    snippet: str,
    published_at: str | None = None,
) -> dict[str, Any]:
    identifier = re.sub(r"[^a-zA-Z0-9]+", "-", f"{platform}-{title}-{company}").strip("-").lower()
    return {
        "id": identifier[:120] or f"{platform}-job",
        "platform": platform,
        "title": _clean_text(title, 120),
        "company": _clean_text(company, 80),
        "location": _clean_text(location, 90),
        "url": url,
        "snippet": _clean_text(snippet, 320),
        "published_at": published_at,
    }


async def _discover_remoteok(query: str, limit: int) -> list[dict[str, Any]]:
    data = await _safe_get_json("https://remoteok.com/api")
    if not isinstance(data, list):
        return []

    items: list[dict[str, Any]] = []
    query_terms = _split_keywords(query)
    for row in data:
        if not isinstance(row, dict):
            continue
        title = str(row.get("position", "")).strip()
        if not title:
            continue

        body = str(row.get("description", ""))
        hay = f"{title} {body}".lower()
        if query_terms and not any(term in hay for term in query_terms):
            continue

        url = str(row.get("url", "") or row.get("apply_url", "")).strip()
        if not url:
            slug = str(row.get("slug", "")).strip()
            if slug:
                url = f"https://remoteok.com/{slug}"
            else:
                continue

        items.append(
            _to_opportunity(
                platform="remoteok",
                title=title,
                company=str(row.get("company", "Unknown")),
                location=str(row.get("location", "Remote")),
                url=url,
                snippet=body or "Remote position",
                published_at=str(row.get("date", "") or "") or None,
            )
        )
        if len(items) >= limit:
            break

    return items


async def _discover_remotive(query: str, limit: int) -> list[dict[str, Any]]:
    encoded = quote_plus(query)
    data = await _safe_get_json(f"https://remotive.com/api/remote-jobs?search={encoded}")
    jobs = data.get("jobs") if isinstance(data, dict) else None
    if not isinstance(jobs, list):
        return []

    items: list[dict[str, Any]] = []
    for row in jobs:
        if not isinstance(row, dict):
            continue
        title = str(row.get("title", "")).strip()
        if not title:
            continue

        items.append(
            _to_opportunity(
                platform="remotive",
                title=title,
                company=str(row.get("company_name", "Unknown")),
                location=str(row.get("candidate_required_location", "Remote")),
                url=str(row.get("url", "")).strip(),
                snippet=str(row.get("description", "Remote role")),
                published_at=str(row.get("publication_date", "") or "") or None,
            )
        )
        if len(items) >= limit:
            break

    return items


async def _discover_arbeitnow(query: str, limit: int) -> list[dict[str, Any]]:
    encoded = quote_plus(query)
    data = await _safe_get_json(f"https://www.arbeitnow.com/api/job-board-api?search={encoded}")
    jobs = data.get("data") if isinstance(data, dict) else None
    if not isinstance(jobs, list):
        return []

    items: list[dict[str, Any]] = []
    for row in jobs:
        if not isinstance(row, dict):
            continue
        title = str(row.get("title", "")).strip()
        if not title:
            continue

        items.append(
            _to_opportunity(
                platform="arbeitnow",
                title=title,
                company=str(row.get("company_name", "Unknown")),
                location=str(row.get("location", "Worldwide")),
                url=str(row.get("url", "")).strip(),
                snippet=str(row.get("description", "Global role")),
                published_at=str(row.get("created_at", "") or "") or None,
            )
        )
        if len(items) >= limit:
            break

    return items


def _search_url(platform: str, query: str, location: str, remote_only: bool) -> str:
    q = quote_plus(query)
    l = quote_plus(location or "Remote")

    if platform == "linkedin":
        remote_filter = "&f_WT=2" if remote_only else ""
        return f"https://www.linkedin.com/jobs/search/?keywords={q}&location={l}{remote_filter}"
    if platform == "upwork":
        return f"https://www.upwork.com/nx/search/jobs/?q={q}"
    if platform == "indeed":
        return f"https://www.indeed.com/jobs?q={q}&l={l}"
    if platform == "glassdoor":
        return f"https://www.glassdoor.com/Job/jobs.htm?sc.keyword={q}&locT=C&locId=1"
    if platform == "wellfound":
        return f"https://wellfound.com/jobs?query={q}"
    if platform == "freelancer":
        return f"https://www.freelancer.com/jobs/{q}/"
    if platform == "weworkremotely":
        return f"https://weworkremotely.com/remote-jobs/search?term={q}"
    return ""


async def _discover_by_scrape(platform: str, query: str, location: str, remote_only: bool, limit: int) -> list[dict[str, Any]]:
    search_url = _search_url(platform, query, location, remote_only)
    if not search_url:
        return []

    html = await _safe_get_text(search_url)
    soup = BeautifulSoup(html, "html.parser")

    query_terms = _split_keywords(query)
    items: list[dict[str, Any]] = []
    seen: set[str] = set()

    for anchor in soup.find_all("a", href=True):
        href = str(anchor.get("href", "")).strip()
        if not href:
            continue

        abs_url = urljoin(search_url, href)
        abs_url = abs_url.split("#", 1)[0]
        if abs_url in seen:
            continue
        if not _job_like_url(platform, abs_url):
            continue

        title = _clean_text(anchor.get_text(" ", strip=True), 140)
        if len(title) < 12:
            continue

        parent = anchor.find_parent(["article", "li", "div", "section"])
        context_text = _clean_text(parent.get_text(" ", strip=True), 420) if parent else ""
        hay = f"{title} {context_text}".lower()
        if query_terms and not any(term in hay for term in query_terms):
            continue

        company = ""
        location_text = location if location else "Worldwide"

        item = _to_opportunity(
            platform=platform,
            title=title,
            company=company or "Unknown",
            location=location_text,
            url=abs_url,
            snippet=context_text or f"Opportunity discovered on {platform}.",
            published_at=datetime.now(timezone.utc).isoformat(),
        )

        items.append(item)
        seen.add(abs_url)
        if len(items) >= limit:
            break

    return items


async def _discover_platform(platform: str, query: str, location: str, remote_only: bool, limit: int) -> list[dict[str, Any]]:
    try:
        if platform == "remoteok":
            return await _discover_remoteok(query, limit)
        if platform == "remotive":
            return await _discover_remotive(query, limit)
        if platform == "arbeitnow":
            return await _discover_arbeitnow(query, limit)
        return await _discover_by_scrape(platform, query, location, remote_only, limit)
    except Exception as exc:
        logger.warning("Opportunity discovery failed for %s: %s", platform, exc)
        return []


@router.post("/interview/generate")
async def generate_interview_session(body: InterviewGenerateBody) -> dict[str, Any]:
    role = body.target_role.strip() or "Software Engineer"
    level = _normalize_level(body.level)
    interview_type = body.interview_type.strip().lower() or "mixed"

    prompt = f"""You are an expert technical interviewer.
Generate a fully AI interview plan as valid JSON only.
Schema:
{{
  "session_title": "string",
  "role": "string",
  "level": "BEGINNER|INTERMEDIATE|ADVANCED|EXPERT",
  "interview_type": "technical|behavioral|system-design|mixed",
  "opening_pitch": "string",
  "questions": [
    {{
      "id": "Q1",
      "question": "string",
      "category": "technical|behavioral|system-design|problem-solving|culture",
      "difficulty": "easy|medium|hard",
      "ideal_answer_points": ["string"],
      "red_flags": ["string"],
      "follow_up": "string"
    }}
  ],
  "coaching_tips": ["string"]
}}
Candidate role: {role}
Seniority level: {level}
Interview type: {interview_type}
Focus skills: {body.focus_skills}
Strong skills: {body.strong_skills}
Weak skills: {body.weak_skills}
Job description: {body.job_description or 'N/A'}
Question count: {body.question_count}
Language: {body.language}
"""

    try:
        data = await call_ollama_json(prompt, temperature=0.45, retry_stricter=True)
        if isinstance(data, dict) and isinstance(data.get("questions"), list) and data.get("questions"):
            questions: list[dict[str, Any]] = []
            for idx, row in enumerate(data["questions"][: body.question_count], start=1):
                if not isinstance(row, dict):
                    continue
                question_text = str(row.get("question", "")).strip()
                if not question_text:
                    continue
                questions.append(
                    {
                        "id": str(row.get("id", f"Q{idx}")),
                        "question": question_text,
                        "category": str(row.get("category", "technical")),
                        "difficulty": str(row.get("difficulty", "medium")),
                        "ideal_answer_points": [str(x) for x in row.get("ideal_answer_points", [])][:6],
                        "red_flags": [str(x) for x in row.get("red_flags", [])][:4],
                        "follow_up": str(row.get("follow_up", "")).strip(),
                    }
                )

            if questions:
                return {
                    "session_title": str(data.get("session_title", f"AI Interview for {role}")),
                    "role": str(data.get("role", role)),
                    "level": str(data.get("level", level)).upper(),
                    "interview_type": str(data.get("interview_type", interview_type)),
                    "opening_pitch": str(data.get("opening_pitch", "Let's start your interview simulation.")),
                    "questions": questions,
                    "coaching_tips": [str(x) for x in data.get("coaching_tips", [])][:8],
                }
    except Exception as exc:
        logger.warning("Interview generation failed, using fallback: %s", exc)

    # Deterministic fallback when the model is unavailable.
    fallback_questions: list[dict[str, Any]] = []
    focus = body.focus_skills or ["communication", "problem solving", "system design", "delivery"]
    for idx in range(min(body.question_count, 10)):
        skill = focus[idx % len(focus)]
        fallback_questions.append(
            {
                "id": f"Q{idx + 1}",
                "question": f"Describe a concrete situation where you applied {skill} in a {role} context.",
                "category": "mixed" if interview_type == "mixed" else interview_type,
                "difficulty": "medium",
                "ideal_answer_points": [
                    "Clear context and constraints",
                    "Specific actions taken",
                    "Measurable outcomes",
                    "Lessons learned",
                ],
                "red_flags": ["Vague answer", "No ownership", "No measurable result"],
                "follow_up": f"What would you improve next time regarding {skill}?",
            }
        )

    return {
        "session_title": f"AI Interview for {role}",
        "role": role,
        "level": level,
        "interview_type": interview_type,
        "opening_pitch": "We will run a realistic AI interview simulation based on your profile.",
        "questions": fallback_questions,
        "coaching_tips": [
            "Use STAR format (Situation, Task, Action, Result).",
            "Quantify outcomes whenever possible.",
            "Connect each answer to business impact.",
        ],
    }


@router.post("/interview/evaluate")
async def evaluate_interview_answer(body: InterviewEvaluateBody) -> dict[str, Any]:
    answer = body.answer.strip()
    if not answer:
        return {
            "score": 0,
            "decision": "fail",
            "strengths": [],
            "gaps": ["No answer provided"],
            "feedback": "Please provide an answer to evaluate.",
            "improved_answer": "",
            "follow_up_question": "",
        }

    prompt = f"""You are an interview assessor.
Return JSON only with this schema:
{{
  "score": 0,
  "decision": "pass|maybe|fail",
  "strengths": ["string"],
  "gaps": ["string"],
  "feedback": "string",
  "improved_answer": "string",
  "follow_up_question": "string"
}}
Role: {body.role}
Level: {_normalize_level(body.level)}
Category: {body.category}
Question: {body.question}
Ideal points: {body.ideal_answer_points}
Job description context: {body.job_description or 'N/A'}
Candidate answer:
{answer}
"""

    try:
        data = await call_ollama_json(prompt, temperature=0.35, retry_stricter=True)
        if isinstance(data, dict):
            score = int(max(0, min(100, int(data.get("score", 0)))))
            decision = str(data.get("decision", "maybe")).lower()
            if decision not in {"pass", "maybe", "fail"}:
                decision = "maybe"
            return {
                "score": score,
                "decision": decision,
                "strengths": [str(x) for x in data.get("strengths", [])][:6],
                "gaps": [str(x) for x in data.get("gaps", [])][:6],
                "feedback": str(data.get("feedback", "")),
                "improved_answer": str(data.get("improved_answer", "")),
                "follow_up_question": str(data.get("follow_up_question", "")),
            }
    except Exception as exc:
        logger.warning("Interview evaluation failed, using fallback: %s", exc)

    # Fallback scoring if model is unavailable.
    word_count = len(answer.split())
    base = 30 if word_count > 20 else 15
    structure_bonus = 20 if any(token in answer.lower() for token in ["result", "impact", "metric", "improve"]) else 0
    detail_bonus = min(30, (word_count // 25) * 10)
    score = min(95, base + structure_bonus + detail_bonus)

    decision = "pass" if score >= 75 else "maybe" if score >= 50 else "fail"
    strengths = ["Good detail level"] if word_count > 40 else ["Answer provided"]
    gaps = [] if score >= 60 else ["Add more concrete measurable outcomes"]

    return {
        "score": score,
        "decision": decision,
        "strengths": strengths,
        "gaps": gaps,
        "feedback": "Model fallback scoring was used. Add more quantified outcomes and ownership details.",
        "improved_answer": "Use STAR format and mention impact with numbers.",
        "follow_up_question": "Can you share a metric that proves your impact?",
    }


@router.post("/profile/improve")
async def improve_cv_linkedin_profile(body: ProfileImproveBody) -> dict[str, Any]:
    cv_text_for_prompt = (body.cv_text or "").strip()
    if not cv_text_for_prompt and body.cv_url:
        cv_text_for_prompt = await _extract_cv_text_from_url(body.cv_url, 6000)

    fallback = _build_profile_improvement_fallback(body, cv_text_for_prompt if cv_text_for_prompt else None)
    target_role = body.target_role.strip() or "Software Engineer"
    normalized_level = _normalize_level(body.level)

    prompt = f"""You are a senior CV and LinkedIn profile optimization coach.
Return valid JSON only using this exact schema:
{{
    "target_role": "string",
    "headline": {{
        "current": "string",
        "improved": "string",
        "actionable_edits": [
            {{"action": "string", "before": "string", "after": "string", "why": "string"}}
        ]
    }},
    "summary": {{
        "current": "string",
        "improved": "string",
        "actionable_edits": [
            {{"action": "string", "before": "string", "after": "string", "why": "string"}}
        ]
    }},
    "skills": {{
        "current": ["string"],
        "prioritized_for_role": ["string"],
        "add": ["string"],
        "deprioritize": ["string"],
        "actionable_edits": [
            {{"action": "string", "before": "string", "after": "string", "why": "string"}}
        ]
    }},
    "role_alignment": {{
        "score": 0,
        "strengths": ["string"],
        "gaps": ["string"],
        "actionable_edits": [
            {{"action": "string", "before": "string", "after": "string", "why": "string"}}
        ]
    }},
    "next_steps": ["string"]
}}
Rules:
- Provide actionable edits, not generic advice.
- Keep all suggestions realistic and evidence-based.
- Prioritize role alignment and recruiter readability.
- Provide 3-6 actionable edits for headline, summary, and skills.
- If current summary is missing and CV text or CV URL is available, generate the summary from CV evidence.
- Language: {body.language}.

Candidate context:
Target role: {target_role}
Level: {normalized_level}
Current headline: {body.current_headline or 'N/A'}
Current summary: {body.current_summary or 'N/A'}
Current skills: {_normalize_skill_names(body.current_skills, body.candidate_skills)}
LinkedIn profile URL: {body.linkedin_url or 'N/A'}
LinkedIn About text (optional free text): {body.linkedin_about or 'N/A'}
CV file URL: {body.cv_url or 'N/A'}
CV text excerpt: {cv_text_for_prompt or 'N/A'}
Job description context: {body.job_description or 'N/A'}
Candidate context metadata: {body.candidate_context}
"""

    try:
        data = await call_ollama_json(prompt, temperature=0.35, retry_stricter=True)
        if isinstance(data, dict):
            headline_raw = data.get("headline") if isinstance(data.get("headline"), dict) else {}
            summary_raw = data.get("summary") if isinstance(data.get("summary"), dict) else {}
            skills_raw = data.get("skills") if isinstance(data.get("skills"), dict) else {}
            role_raw = data.get("role_alignment") if isinstance(data.get("role_alignment"), dict) else {}

            role_score = role_raw.get("score")
            try:
                score = int(max(0, min(100, int(role_score))))
            except (TypeError, ValueError):
                score = int(fallback["role_alignment"]["score"])

            return {
                "target_role": _clean_text(
                    str(data.get("target_role", fallback["target_role"])).strip() or fallback["target_role"],
                    90,
                ),
                "headline": {
                    "current": _clean_text(
                        str(headline_raw.get("current", fallback["headline"]["current"])).strip()
                        or fallback["headline"]["current"],
                        180,
                    ),
                    "improved": _clean_text(
                        str(headline_raw.get("improved", fallback["headline"]["improved"])).strip()
                        or fallback["headline"]["improved"],
                        180,
                    ),
                    "actionable_edits": _actionable_edits(headline_raw.get("actionable_edits"), 8)
                    or fallback["headline"]["actionable_edits"],
                },
                "summary": {
                    "current": _clean_text(
                        str(summary_raw.get("current", fallback["summary"]["current"])).strip()
                        or fallback["summary"]["current"],
                        520,
                    ),
                    "improved": _clean_text(
                        str(summary_raw.get("improved", fallback["summary"]["improved"])).strip()
                        or fallback["summary"]["improved"],
                        620,
                    ),
                    "actionable_edits": _actionable_edits(summary_raw.get("actionable_edits"), 8)
                    or fallback["summary"]["actionable_edits"],
                },
                "skills": {
                    "current": _string_list(skills_raw.get("current"), 20) or fallback["skills"]["current"],
                    "prioritized_for_role": _string_list(skills_raw.get("prioritized_for_role"), 12)
                    or fallback["skills"]["prioritized_for_role"],
                    "add": _string_list(skills_raw.get("add"), 10) or fallback["skills"]["add"],
                    "deprioritize": _string_list(skills_raw.get("deprioritize"), 10)
                    or fallback["skills"]["deprioritize"],
                    "actionable_edits": _actionable_edits(skills_raw.get("actionable_edits"), 8)
                    or fallback["skills"]["actionable_edits"],
                },
                "role_alignment": {
                    "score": score,
                    "strengths": _string_list(role_raw.get("strengths"), 8) or fallback["role_alignment"]["strengths"],
                    "gaps": _string_list(role_raw.get("gaps"), 8) or fallback["role_alignment"]["gaps"],
                    "actionable_edits": _actionable_edits(role_raw.get("actionable_edits"), 8)
                    or fallback["role_alignment"]["actionable_edits"],
                },
                "next_steps": _string_list(data.get("next_steps"), 10) or fallback["next_steps"],
            }
    except Exception as exc:
        logger.warning("Profile improvement generation failed, using fallback: %s", exc)

    return fallback


@router.post("/opportunities/discover")
async def discover_opportunities(body: OpportunityDiscoverBody) -> dict[str, Any]:
    platforms = _normalize_platforms(body.platforms)
    query = _build_query(body.position, body.level, body.location, body.remote_only)
    query_terms = _split_keywords(query)

    per_platform_limit = max(3, min(12, body.max_results // max(1, len(platforms)) + 1))

    tasks = [
        _discover_platform(platform, query, body.location, body.remote_only, per_platform_limit)
        for platform in platforms
    ]
    gathered = await asyncio.gather(*tasks, return_exceptions=True)

    opportunities: list[dict[str, Any]] = []
    for platform, result in zip(platforms, gathered, strict=False):
        if isinstance(result, Exception):
            logger.warning("Platform discovery error for %s: %s", platform, result)
            continue
        for item in result:
            score, hits = _score_opportunity(item, body.candidate_skills, query_terms)
            item["match_score"] = score
            item["matched_skills"] = hits
            opportunities.append(item)

    # Deduplicate by URL and keep the highest score.
    by_url: dict[str, dict[str, Any]] = {}
    for item in opportunities:
        url = str(item.get("url", "")).strip()
        if not url:
            continue
        current = by_url.get(url)
        if current is None or int(item.get("match_score", 0)) > int(current.get("match_score", 0)):
            by_url[url] = item

    final_items = sorted(
        by_url.values(),
        key=lambda x: (int(x.get("match_score", 0)), str(x.get("published_at", ""))),
        reverse=True,
    )[: body.max_results]

    # Guarantee discoverability even if scraping is blocked.
    if not final_items:
        for platform in platforms:
            final_items.append(
                {
                    "id": f"{platform}-search-link",
                    "platform": platform,
                    "title": f"Search {query} on {platform}",
                    "company": "",
                    "location": body.location,
                    "url": _search_url(platform, query, body.location, body.remote_only),
                    "snippet": "Direct search link generated because scraping/API did not return public listings.",
                    "published_at": None,
                    "match_score": 0,
                    "matched_skills": [],
                    "is_search_link": True,
                }
            )

    jd_match: dict[str, Any] | None = None
    if body.job_description and body.job_description.strip():
        try:
            jd_match = await match_job_description(
                job_url=None,
                job_description=body.job_description,
                candidate_skills=body.candidate_skills,
                candidate_context=body.candidate_context,
            )
        except Exception as exc:
            logger.warning("Job-description matching failed: %s", exc)

    search_links = {
        platform: _search_url(platform, query, body.location, body.remote_only)
        for platform in platforms
    }

    return {
        "query": {
            "position": body.position,
            "level": _normalize_level(body.level),
            "location": body.location,
            "remote_only": body.remote_only,
            "search_query": query,
        },
        "platforms_used": platforms,
        "search_links": search_links,
        "total_found": len(final_items),
        "opportunities": final_items,
        "job_description_match": jd_match,
    }


# ---------------------------------------------------------------------------
# Learning Plan — request model
# ---------------------------------------------------------------------------

class WeakSkill(BaseModel):
    name: str
    score: float = Field(ge=0, le=100, description="Current proficiency 0-100")


class LearningPlanBody(BaseModel):
    candidate_id: str
    target_role: str = "Software Engineer"
    experience_level: str = "intermediate"   # beginner | intermediate | advanced | expert
    weak_skills: list[WeakSkill] = Field(default_factory=list)
    language: str = "en"                     # en | fr | ar
    timezone: str = "UTC"
    preferred_learning_style: str = "video"  # video | reading | hands-on


# ---------------------------------------------------------------------------
# Helpers — deterministic fallback
# ---------------------------------------------------------------------------

_PLATFORM_SEARCH_URLS: dict[str, str] = {
    "Udemy": "https://www.udemy.com/courses/search/?q={q}",
    "Coursera": "https://www.coursera.org/search?query={q}",
    "edX": "https://www.edx.org/search?q={q}",
    "freeCodeCamp": "https://www.freecodecamp.org/news/search/?query={q}",
}

_OFFICIAL_DOCS: dict[str, str] = {
    "react": "https://react.dev/learn",
    "angular": "https://angular.dev/overview",
    "vue": "https://vuejs.org/guide/introduction",
    "typescript": "https://www.typescriptlang.org/docs/",
    "javascript": "https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide",
    "python": "https://docs.python.org/3/tutorial/",
    "java": "https://docs.oracle.com/en/java/javase/21/docs/api/",
    "spring": "https://spring.io/guides",
    "docker": "https://docs.docker.com/get-started/",
    "kubernetes": "https://kubernetes.io/docs/tutorials/",
    "git": "https://git-scm.com/doc",
    "sql": "https://www.w3schools.com/sql/",
    "postgresql": "https://www.postgresql.org/docs/current/tutorial.html",
    "mongodb": "https://www.mongodb.com/docs/manual/tutorial/",
    "fastapi": "https://fastapi.tiangolo.com/tutorial/",
    "django": "https://docs.djangoproject.com/en/stable/intro/tutorial01/",
    "node": "https://nodejs.org/en/docs/guides/",
    "aws": "https://docs.aws.amazon.com/",
    "ci/cd": "https://docs.github.com/en/actions",
    "testing": "https://jestjs.io/docs/getting-started",
}

_PRIORITY_COMMUNITIES: dict[str, dict] = {
    "react": {"name": "Reactiflux Discord", "url": "https://www.reactiflux.com/"},
    "python": {"name": "Python Discord", "url": "https://pythondiscord.com/"},
    "angular": {"name": "Angular Community", "url": "https://discord.com/invite/angular"},
    "default": {"name": "Dev.to Community", "url": "https://dev.to/"},
}


def _platform_search_url(platform: str, skill: str) -> str:
    template = _PLATFORM_SEARCH_URLS.get(platform, "https://www.google.com/search?q={q}")
    return template.format(q=quote_plus(skill))


def _skill_level_from_score(score: float) -> str:
    if score < 30:
        return "beginner"
    if score < 60:
        return "intermediate"
    return "advanced"


def _priority_from_score(score: float) -> str:
    if score < 30:
        return "high"
    if score < 60:
        return "medium"
    return "low"


def _required_level(score: float) -> int:
    """Estimate required role proficiency (1–10) from current score."""
    return max(7, min(10, round(10 - score / 20)))


def _gap(score: float) -> int:
    required = _required_level(score)
    current = round(score / 10)
    return max(0, required - current)


def _community_for_skill(skill_name: str) -> dict[str, str]:
    key = skill_name.strip().lower()
    for k, v in _PRIORITY_COMMUNITIES.items():
        if k in key:
            return v
    return _PRIORITY_COMMUNITIES["default"]


def _build_learning_plan_fallback(body: LearningPlanBody) -> dict[str, Any]:
    """Deterministic fallback when the LLM is unavailable."""
    from datetime import date, timedelta

    today = date.today()
    checkpoint = (today + timedelta(days=14)).isoformat()

    # Sort weakest skills first
    sorted_skills = sorted(body.weak_skills, key=lambda s: s.score)
    skill_names = [s.name for s in sorted_skills]
    role = body.target_role.strip() or "Software Engineer"

    # Skill-gap analysis
    gap_rows: list[dict[str, Any]] = []
    critical_blockers: list[str] = []
    for s in sorted_skills:
        req = _required_level(s.score)
        cur = round(s.score / 10)
        g = max(0, req - cur)
        pri = "critical" if g >= 5 else ("high" if g >= 3 else ("medium" if g >= 1 else "low"))
        if pri == "critical":
            critical_blockers.append(s.name)
        gap_rows.append({
            "skill": s.name,
            "required_level": req,
            "current_level": cur,
            "gap": g,
            "priority": pri,
        })

    avg_score = (sum(s.score for s in sorted_skills) / len(sorted_skills)) if sorted_skills else 50
    readiness_score = max(5, min(90, round(avg_score * 0.85)))

    # Roadmap — 3 phases
    phase_size = max(1, len(sorted_skills) // 3 or 1)
    phases: list[dict[str, Any]] = []
    chunks = [
        sorted_skills[:phase_size],
        sorted_skills[phase_size: phase_size * 2],
        sorted_skills[phase_size * 2:],
    ]
    phase_labels = ["Foundations", "Core Proficiency", "Advanced & Portfolio"]
    week_cursor = 1
    for idx, (chunk, label) in enumerate(zip(chunks, phase_labels), start=1):
        if not chunk:
            continue
        focus_skills = [s.name for s in chunk]
        duration_weeks = max(2, len(chunk) * 2)
        phases.append({
            "phase": f"Phase {idx} — {label}",
            "duration": f"{duration_weeks} weeks",
            "start_week": week_cursor,
            "end_week": week_cursor + duration_weeks - 1,
            "focus": focus_skills,
            "goals": [f"Reach working proficiency in {', '.join(focus_skills)}"],
            "success_criteria": [
                f"Complete at least 1 hands-on project using {focus_skills[0]}",
                "Pass the phase micro-assessment with ≥70%",
            ],
        })
        week_cursor += duration_weeks

    # Formations
    formations: list[dict[str, Any]] = []
    for s in sorted_skills:
        lvl = _skill_level_from_score(s.score)
        pri = _priority_from_score(s.score)
        skill_key = s.name.strip().lower()
        doc_url = next((v for k, v in _OFFICIAL_DOCS.items() if k in skill_key), None)

        courses: list[dict[str, Any]] = [
            {
                "title": f"Complete {s.name} Bootcamp",
                "platform": "Udemy",
                "url": _platform_search_url("Udemy", s.name),
                "provider": "Udemy",
                "duration": "20-30 hours",
                "level": lvl,
                "reason": f"Comprehensive course matching your {lvl} level to close the gap quickly.",
            },
            {
                "title": f"{s.name} for Developers",
                "platform": "Coursera",
                "url": _platform_search_url("Coursera", s.name),
                "provider": "Coursera",
                "duration": "8-12 hours",
                "level": lvl,
                "reason": f"Structured curriculum with projects to build {s.name} confidence.",
            },
        ]
        if doc_url:
            courses.append({
                "title": f"Official {s.name} Documentation & Tutorial",
                "platform": "Official Docs",
                "url": doc_url,
                "provider": s.name,
                "duration": "Self-paced",
                "level": lvl,
                "reason": "Primary source — always up-to-date and directly reflects real-world API usage.",
            })

        formations.append({"skill": s.name, "priority": pri, "courses": courses})

    # Reinforcement
    reinforcement: list[dict[str, Any]] = []
    for i, s in enumerate(sorted_skills[:4]):
        diff = "easy" if s.score < 30 else ("medium" if s.score < 60 else "hard")
        reinforcement.append({
            "type": "project" if i % 2 == 0 else "practice",
            "title": f"Build a mini-{s.name} demo app",
            "skills": [s.name],
            "difficulty": diff,
            "duration_hours": 4 if diff == "easy" else (8 if diff == "medium" else 16),
            "description": (
                f"Create a minimal working application that exercises core {s.name} concepts. "
                f"Focus on real patterns used in {role} job descriptions."
            ),
            "success_metric": f"App runs, code is on GitHub, README explains what you built.",
        })

    # Capstone project
    top_skills = skill_names[:5] or ["problem-solving"]
    project_plan: dict[str, Any] = {
        "title": f"{role} Showcase Project",
        "description": (
            f"A full end-to-end project that demonstrates your ability to work as a {role}. "
            f"Combines your weakest skills ({', '.join(top_skills[:3])}) into one cohesive deliverable "
            "that can be pinned on your GitHub profile."
        ),
        "features": [
            f"Feature using {top_skills[0]}",
            "RESTful API or UI layer",
            "Basic tests and CI/CD pipeline",
            "Deployed to a free cloud platform (Render / Vercel / Railway)",
        ],
        "tech_stack": top_skills,
        "steps": [
            {"step": 1, "title": "Project setup & repo", "description": "Init repo, README, folder structure.", "estimated_days": 1},
            {"step": 2, "title": f"Core {top_skills[0]} implementation", "description": f"Build the main feature using {top_skills[0]}.", "estimated_days": 5},
            {"step": 3, "title": "Integration layer", "description": "Connect all components, add basic error handling.", "estimated_days": 3},
            {"step": 4, "title": "Testing & documentation", "description": "Write unit tests, update README with screenshots.", "estimated_days": 2},
            {"step": 5, "title": "Deployment", "description": "Deploy to free hosting, add live URL to GitHub.", "estimated_days": 1},
        ],
        "portfolio_outcome": (
            f"Proves to a recruiter that you can independently build and ship a {role} project "
            f"using {', '.join(top_skills[:3])}."
        ),
    }

    # Daily plan (14 days)
    daily_plan: list[dict[str, Any]] = []
    for day in range(1, 15):
        skill_for_day = sorted_skills[(day - 1) % max(1, len(sorted_skills))]
        phase_for_day = 1 if day <= 5 else (2 if day <= 10 else 3)
        daily_plan.append({
            "day": day,
            "phase": phase_for_day,
            "theme": f"{skill_for_day.name} — Day {((day - 1) % 5) + 1}",
            "tasks": [
                f"Watch/read 1 learning unit on {skill_for_day.name}",
                "Take notes on 3 key concepts",
                "Complete 1 hands-on exercise",
            ],
            "resources": [_platform_search_url("Udemy", skill_for_day.name)],
            "estimated_time": "2-3 hours",
            "deliverable": f"Completed exercise pushed to GitHub or saved locally.",
        })

    # Milestones
    milestones: list[dict[str, Any]] = [
        {
            "id": "m1",
            "title": f"{sorted_skills[0].name if sorted_skills else 'Foundation'} Fundamentals Unlocked",
            "trigger": "Complete Phase 1 and first reinforcement project",
            "badge": "foundations-complete",
            "phase": 1,
            "reward_message": "You've closed the most critical skill gap. Phase 2 will build on this.",
        },
        {
            "id": "m2",
            "title": "Mid-Plan Checkpoint Reached",
            "trigger": "Complete Phase 2 and pass the mid-plan assessment",
            "badge": "halfway-there",
            "phase": 2,
            "reward_message": "Core proficiency achieved. You're now hireable for junior roles targeting this stack.",
        },
        {
            "id": "m3",
            "title": "Portfolio-Ready",
            "trigger": "Deploy the capstone project with a live URL",
            "badge": "portfolio-ready",
            "phase": 3,
            "reward_message": f"Your GitHub now demonstrates a working {role} project. Start applying.",
        },
    ]

    # Assessments
    assessments: list[dict[str, Any]] = [
        {
            "skill": s.name,
            "phase": 1 if i < phase_size else (2 if i < phase_size * 2 else 3),
            "type": "code-challenge",
            "passing_score": 70,
            "description": f"Short challenge covering {s.name} fundamentals — 5-10 exercises.",
            "resource_url": _platform_search_url("freeCodeCamp", s.name),
        }
        for i, s in enumerate(sorted_skills[:6])
    ]

    # Re-evaluation
    re_evaluation: dict[str, Any] = {
        "trigger_after_days": 14,
        "quiz_score_threshold": 60,
        "re_evaluate_skills": skill_names[:3],
        "next_checkpoint_date": checkpoint,
    }

    # Mentor profile
    community = _community_for_skill(skill_names[0] if skill_names else "")
    mentor_profile: dict[str, Any] = {
        "ideal_mentor_skills": skill_names[:3],
        "mentor_type": "senior-dev",
        "recommended_communities": [
            {**community, "why": "Active community with daily Q&A and code reviews."},
            {"name": "LinkedIn Learning Groups", "url": "https://www.linkedin.com/learning/", "why": "Professional network for role-specific peer learning."},
        ],
    }

    # Market alignment
    market_alignment: dict[str, Any] = {
        "top_hiring_companies": ["Startups", "Mid-size tech firms", "Consulting agencies"],
        "avg_salary_range": "$60k - $100k (varies by region and seniority)",
        "most_requested_skills_in_job_posts": skill_names[:5],
        "job_search_keywords": [role, f"Junior {role}", f"{role} Developer"],
        "time_to_first_interview_estimate": f"~{week_cursor} weeks after completing this plan",
    }

    # Weekly check-ins
    weekly_checkins: list[dict[str, Any]] = [
        {
            "week": w,
            "focus_skill": sorted_skills[(w - 1) % max(1, len(sorted_skills))].name if sorted_skills else role,
            "questions": [
                "What concept did you struggle with most this week?",
                "Did you complete your daily plan? If not, why?",
                "What's one thing you could teach someone else right now?",
                "Did you push any code to GitHub this week?",
            ],
        }
        for w in range(1, (week_cursor // 7) + 2)
    ]

    # Summary
    main_gaps = [s.name for s in sorted_skills[:3]]
    profile_eval = (
        f"Candidate targeting {role} at {body.experience_level} level. "
        f"Main skill gaps are: {', '.join(main_gaps)}. "
        f"Estimated readiness score: {readiness_score}/100 — focused, structured study can close these gaps in {week_cursor} weeks."
    )

    return {
        "generated_at": today.isoformat(),
        "candidate_id": body.candidate_id,
        "target_role": role,
        "language": body.language,
        "summary": {
            "profile_evaluation": profile_eval,
            "main_gaps": main_gaps,
            "estimated_time_to_ready": f"{week_cursor} weeks",
        },
        "skill_gap_analysis": {
            "target_role_requirements": gap_rows,
            "readiness_score": readiness_score,
            "critical_blockers": critical_blockers,
        },
        "roadmap": phases,
        "formations": formations,
        "reinforcement": reinforcement,
        "project_plan": project_plan,
        "daily_plan": daily_plan,
        "milestones": milestones,
        "assessments": assessments,
        "re_evaluation": re_evaluation,
        "mentor_profile": mentor_profile,
        "market_alignment": market_alignment,
        "weekly_checkins": weekly_checkins,
        "progress": {
            "started_at": today.isoformat(),
            "current_phase": 1,
            "completed_courses": [],
            "completed_projects": [],
            "overall_completion_pct": 0,
        },
        "localization": {
            "language": body.language,
            "timezone": body.timezone,
            "preferred_learning_style": body.preferred_learning_style,
        },
    }


# ---------------------------------------------------------------------------
# Learning Plan — endpoint
# ---------------------------------------------------------------------------

@router.post("/learning-plan")
async def generate_learning_plan(body: LearningPlanBody) -> dict[str, Any]:
    """Generate a personalized learning plan from a candidate's weak skills.

    Uses the local LLM (Ollama) to produce a rich, structured plan.
    Falls back to a deterministic plan when the model is unavailable.
    """
    if not body.weak_skills:
        return {
            "error": "weak_skills list is empty — provide at least one skill with its score.",
            "candidate_id": body.candidate_id,
        }

    from datetime import date as _date

    today_str = _date.today().isoformat()
    level = _normalize_level(body.experience_level)

    # Sort weakest first so the LLM focuses on the highest-priority gaps
    sorted_weak = sorted(body.weak_skills, key=lambda s: s.score)
    skills_json = [{"name": s.name, "score": s.score} for s in sorted_weak]

    prompt = f"""You are an elite AI learning coach integrated into TalentPredict.
Generate a complete personalized learning plan. Return ONLY valid JSON — no markdown, no prose.

## Candidate Profile
candidateId: {body.candidate_id}
targetRole: {body.target_role}
experienceLevel: {level}
language: {body.language}
timezone: {body.timezone}
preferredLearningStyle: {body.preferred_learning_style}
todayDate: {today_str}

## Weak Skills (sorted by priority — lowest score first)
{skills_json}

## STRICT URL RULES
- DO NOT invent URLs
- Udemy search:       https://www.udemy.com/courses/search/?q=SKILL
- Coursera search:    https://www.coursera.org/search?query=SKILL
- edX search:        https://www.edx.org/search?q=SKILL
- freeCodeCamp:       https://www.freecodecamp.org/news/search/?query=SKILL
- When you know the official documentation URL with certainty, use it
- Otherwise use the platform search URL above

## Required JSON Schema (COPY THIS EXACTLY, fill in values)
{{
  "generated_at": "{today_str}",
  "candidate_id": "{body.candidate_id}",
  "target_role": "{body.target_role}",
  "language": "{body.language}",
  "summary": {{
    "profile_evaluation": "2-3 sentence honest assessment",
    "main_gaps": ["skill1", "skill2"],
    "estimated_time_to_ready": "X weeks"
  }},
  "skill_gap_analysis": {{
    "target_role_requirements": [
      {{"skill": "name", "required_level": 8, "current_level": 3, "gap": 5, "priority": "critical|high|medium|low"}}
    ],
    "readiness_score": 40,
    "critical_blockers": ["skill"]
  }},
  "roadmap": [
    {{
      "phase": "Phase 1 — Foundations",
      "duration": "2-3 weeks",
      "start_week": 1,
      "end_week": 3,
      "focus": ["skill1"],
      "goals": ["concrete measurable goal"],
      "success_criteria": ["how to know phase is complete"]
    }}
  ],
  "formations": [
    {{
      "skill": "skill name",
      "priority": "high|medium|low",
      "courses": [
        {{
          "title": "course title",
          "platform": "Udemy|Coursera|edX|freeCodeCamp|Official Docs",
          "url": "real URL",
          "provider": "instructor or org",
          "duration": "12 hours",
          "level": "beginner|intermediate|advanced",
          "reason": "why this course for this candidate"
        }}
      ]
    }}
  ],
  "reinforcement": [
    {{
      "type": "project|practice|challenge",
      "title": "project title",
      "skills": ["skill"],
      "difficulty": "easy|medium|hard",
      "duration_hours": 4,
      "description": "specific actionable description",
      "success_metric": "how to know it is done well"
    }}
  ],
  "project_plan": {{
    "title": "Capstone project title",
    "description": "what and why",
    "features": ["feature"],
    "tech_stack": ["tech"],
    "steps": [
      {{"step": 1, "title": "step title", "description": "what to do", "estimated_days": 2}}
    ],
    "portfolio_outcome": "what this proves to a recruiter"
  }},
  "daily_plan": [
    {{
      "day": 1,
      "phase": 1,
      "theme": "what today is about",
      "tasks": ["specific actionable task"],
      "resources": ["course URL"],
      "estimated_time": "2-3 hours",
      "deliverable": "what you produce today"
    }}
  ],
  "milestones": [
    {{
      "id": "m1",
      "title": "milestone title",
      "trigger": "what must be completed",
      "badge": "badge-slug",
      "phase": 1,
      "reward_message": "what this unlocks"
    }}
  ],
  "assessments": [
    {{
      "skill": "skill name",
      "phase": 1,
      "type": "quiz|code-challenge|project-review",
      "passing_score": 70,
      "description": "what the assessment covers",
      "resource_url": "URL"
    }}
  ],
  "re_evaluation": {{
    "trigger_after_days": 14,
    "quiz_score_threshold": 60,
    "re_evaluate_skills": ["skill"],
    "next_checkpoint_date": "YYYY-MM-DD"
  }},
  "mentor_profile": {{
    "ideal_mentor_skills": ["skill"],
    "mentor_type": "senior-dev|bootcamp-grad|career-changer",
    "recommended_communities": [
      {{"name": "community name", "url": "real URL", "why": "why this fits"}}
    ]
  }},
  "market_alignment": {{
    "top_hiring_companies": ["company"],
    "avg_salary_range": "$70k - $110k",
    "most_requested_skills_in_job_posts": ["skill"],
    "job_search_keywords": ["keyword"],
    "time_to_first_interview_estimate": "3 months"
  }},
  "weekly_checkins": [
    {{
      "week": 1,
      "focus_skill": "skill",
      "questions": [
        "What concept did you struggle with most this week?",
        "Did you complete your daily plan? If not, why?",
        "What's one thing you could teach someone else right now?"
      ]
    }}
  ],
  "progress": {{
    "started_at": "{today_str}",
    "current_phase": 1,
    "completed_courses": [],
    "completed_projects": [],
    "overall_completion_pct": 0
  }},
  "localization": {{
    "language": "{body.language}",
    "timezone": "{body.timezone}",
    "preferred_learning_style": "{body.preferred_learning_style}"
  }}
}}
"""

    fallback = _build_learning_plan_fallback(body)

    try:
        data = await call_ollama_json(prompt, temperature=0.4, retry_stricter=True)
        if not isinstance(data, dict):
            logger.warning("Learning plan LLM returned non-dict — using fallback")
            return fallback

        # Validate critical top-level sections; fall back to deterministic defaults
        # for any section the model failed to produce correctly.
        def _safe_list(val: Any, min_len: int = 0) -> list:
            return val if isinstance(val, list) and len(val) >= min_len else []

        def _safe_dict(val: Any) -> dict:
            return val if isinstance(val, dict) else {}

        summary = _safe_dict(data.get("summary"))
        skill_gap = _safe_dict(data.get("skill_gap_analysis"))
        roadmap = _safe_list(data.get("roadmap"), 1)
        formations = _safe_list(data.get("formations"), 1)
        reinforcement = _safe_list(data.get("reinforcement"))
        project = _safe_dict(data.get("project_plan"))
        daily = _safe_list(data.get("daily_plan"))
        milestones = _safe_list(data.get("milestones"))
        assessments = _safe_list(data.get("assessments"))
        re_eval = _safe_dict(data.get("re_evaluation"))
        mentor = _safe_dict(data.get("mentor_profile"))
        market = _safe_dict(data.get("market_alignment"))
        checkins = _safe_list(data.get("weekly_checkins"))
        progress = _safe_dict(data.get("progress"))
        loc = _safe_dict(data.get("localization"))

        # If key sections are empty the LLM output is not usable — use fallback
        if not roadmap or not formations:
            logger.warning("Learning plan LLM output missing roadmap/formations — using fallback")
            return fallback

        return {
            "generated_at": str(data.get("generated_at", today_str)),
            "candidate_id": body.candidate_id,
            "target_role": str(data.get("target_role", body.target_role)),
            "language": body.language,
            "summary": {
                "profile_evaluation": str(summary.get("profile_evaluation", fallback["summary"]["profile_evaluation"])),
                "main_gaps": _string_list(summary.get("main_gaps"), 5) or fallback["summary"]["main_gaps"],
                "estimated_time_to_ready": str(summary.get("estimated_time_to_ready", fallback["summary"]["estimated_time_to_ready"])),
            },
            "skill_gap_analysis": {
                "target_role_requirements": _safe_list(skill_gap.get("target_role_requirements"))
                    or fallback["skill_gap_analysis"]["target_role_requirements"],
                "readiness_score": max(0, min(100, int(skill_gap.get("readiness_score", fallback["skill_gap_analysis"]["readiness_score"]) or 0))),
                "critical_blockers": _string_list(skill_gap.get("critical_blockers"), 5) or fallback["skill_gap_analysis"]["critical_blockers"],
            },
            "roadmap": roadmap,
            "formations": formations,
            "reinforcement": reinforcement or fallback["reinforcement"],
            "project_plan": project or fallback["project_plan"],
            "daily_plan": daily or fallback["daily_plan"],
            "milestones": milestones or fallback["milestones"],
            "assessments": assessments or fallback["assessments"],
            "re_evaluation": re_eval or fallback["re_evaluation"],
            "mentor_profile": mentor or fallback["mentor_profile"],
            "market_alignment": market or fallback["market_alignment"],
            "weekly_checkins": checkins or fallback["weekly_checkins"],
            "progress": progress or fallback["progress"],
            "localization": loc or fallback["localization"],
        }

    except Exception as exc:
        logger.warning("Learning plan generation failed — using fallback: %s", exc)
        return fallback
